  ...
  return (
    <>
      <PageIntro
        ...
        actions={<button type="button" onClick={createNew} className="inline-flex items-center gap-2 rounded-xl border border-[#dbe5f2] bg-white px-4 py-2.5 text-sm font-semibold text-blue-700 transition hover:bg-blue-50 dark:border-blue-500/30 dark:bg-slate-950 dark:text-blue-200"><Plus className="h-4 w-4" /> New link</button>}
      />
      <div className="grid gap-5 2xl:grid-cols-[400px_minmax(360px,1fr)_minmax(440px,1.25fr)]">
        <Panel className="h-fit">
          ...
          <div className="mt-5 space-y-3">
            <label className="block text-sm font-semibold">Title<input value={form.title} onChange={(event) => setForm({ ...form, title: event.target.value })} placeholder="Disaster preparedness seminar recording" className="mt-1.5 w-full rounded-xl border border-[#dfe7f1] bg-[#fbfcfe] px-3 py-2.5 font-normal outline-none focus:border-blue-500 dark:border-slate-700 dark:bg-slate-900" /></label>
            <label className="block text-sm font-semibold">Link / URL<input type="url" value={form.url} onChange={(event) => setForm({ ...form, url: event.target.value })} placeholder="https://youtube.com/watch?v=..." className="mt-1.5 w-full rounded-xl border border-[#dfe7f1] bg-[#fbfcfe] px-3 py-2.5 font-normal outline-none focus:border-blue-500 dark:border-slate-700 dark:bg-slate-900" /></label>
            <div className="grid gap-2 sm:grid-cols-[1fr_auto]">
              <label className="block text-sm font-semibold">Category / type<select value={form.category} onChange={(event) => setForm({ ...form, category: event.target.value as MaterialCategory })} className="mt-1.5 w-full rounded-xl border border-[#dfe7f1] bg-[#fbfcfe] px-3 py-2.5 font-normal dark:border-slate-700 dark:bg-slate-900">{CATEGORIES.map((value) => <option key={value}>{value}</option>)}</select></label>
            ...
            <label className="block text-sm font-semibold">Description<textarea value={form.description} onChange={(event) => setForm({ ...form, description: event.target.value })} rows={3} placeholder="Learning objective and access instructions" className="mt-1.5 w-full rounded-xl border border-[#dfe7f1] bg-[#fbfcfe] px-3 py-2.5 font-normal outline-none focus:border-blue-500 dark:border-slate-700 dark:bg-slate-900" /></label>
            <div className="grid gap-3 sm:grid-cols-2">
              <label className="block text-sm font-semibold">Assigned phase / component<select value={form.audience} onChange={(event) => setForm({ ...form, audience: event.target.value as MaterialAudience })} className="mt-1.5 w-full rounded-xl border border-[#dfe7f1] bg-[#fbfcfe] px-3 py-2.5 font-normal dark:border-slate-700 dark:bg-slate-900">{AUDIENCES.map((value) => <option key={value}>{value}</option>)}</select></label>
              <label className="block text-sm font-semibold">Visibility<select value={form.visibility} onChange={(event) => setForm({ ...form, visibility: event.target.value as MaterialVisibility })} className="mt-1.5 w-full rounded-xl border border-[#dfe7f1] bg-[#fbfcfe] px-3 py-2.5 font-normal dark:border-slate-700 dark:bg-slate-900">{VISIBILITY.map((value) => <option key={value}>{value}</option>)}</select></label>
              <label className="block text-sm font-semibold">Session date<input type="date" value={form.sessionDate} onChange={(event) => setForm({ ...form, sessionDate: event.target.value })} className="mt-1.5 w-full rounded-xl border border-[#dfe7f1] bg-[#fbfcfe] px-3 py-2.5 font-normal dark:border-slate-700 dark:bg-slate-900" /></label>
              <label className="block text-sm font-semibold">Speaker / facilitator<input value={form.speaker} onChange={(event) => setForm({ ...form, speaker: event.target.value })} className="mt-1.5 w-full rounded-xl border border-[#dfe7f1] bg-[#fbfcfe] px-3 py-2.5 font-normal dark:border-slate-700 dark:bg-slate-900" /></label>
            </div>
            <label className="block text-sm font-semibold">Related assessment / activity<input value={form.relatedActivity} onChange={(event) => setForm({ ...form, relatedActivity: event.target.value })} placeholder="Reflection output or assessment" className="mt-1.5 w-full rounded-xl border border-[#dfe7f1] bg-[#fbfcfe] px-3 py-2.5 font-normal dark:border-slate-700 dark:bg-slate-900" /></label>
            <label className="block text-sm font-semibold">Tags<input value={tags} onChange={(event) => setTags(event.target.value)} placeholder="orientation, DRRM, video" className="mt-1.5 w-full rounded-xl border border-[#dfe7f1] bg-[#fbfcfe] px-3 py-2.5 font-normal dark:border-slate-700 dark:bg-slate-900" /></label>
            <div className="flex gap-2 pt-2">
              ...
              <button type="button" onClick={save} className="inline-flex flex-1 items-center justify-center gap-2 rounded-xl bg-blue-700 px-3 py-3 text-sm font-semibold text-white shadow-lg shadow-blue-700/15"><Save className="h-4 w-4" /> Save</button>
            </div>
          </div>
        </Panel>
        <Panel className="h-fit">
          ...
          <div className="mt-4 space-y-3">
            {filtered.length ? filtered.map((material) => (
              <article key={material.id} className={`rounded-xl border p-3 transition ${selected?.id === material.id ? 'border-blue-300 bg-blue-50 shadow-sm dark:bg-blue-500/10' : 'border-[#e6ecf4] hover:border-blue-200 hover:bg-slate-50 dark:border-slate-800'}`}>
